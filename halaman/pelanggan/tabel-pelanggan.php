    ...
    <tbody>
        ...
        while ($data = mysqli_fetch_array($query)) {
            ...
            <tr>
                ...
                <td><?= $data['kontak'] ?></td>
                <td>
                    <button data-id="<?= $data['id'] ?>" data-name="<?= $data['nama_pelanggan'] ?>" id="edit" type="button"
                        class="btn btn-primary">Edit</button>
                    <button data-id="<?= $data['id'] ?>" data-name="<?= $data['nama_pelanggan'] ?>" id="delete"
...
<script>
    $(document).ready(function () {
        ...
        $('#tabel-data').on('click', '#edit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'halaman/pelanggan/edit-pelanggan.php',
        ...
        $('#tabel-data').on('click', '#delete', function () {
            ...
            alertify.confirm('Hapus', 'Apakah anda yakin ingin menghapus pelanggan ' + name + '?', function () {
                $.ajax({
                    type: 'POST',
                    url: 'aksi.php?aksi=hapus-pelanggan',
                    data: 'id=' + id,
                    success: function (data) {
